...
fun findUrl(s: IRCString, index: Int): Pair<Int, Int>? {
    var urlStart = index
    while (true) {
        ...
        while (true) {
            if (urlStart == s.string.length)
                return null
            when (s.string[urlStart++].category) {
...
private fun skipUrl(s: IRCString, index: Int): Int? {
    val schemes = listOf("http://", "https://")
    for (scheme in schemes) {
        val schemeEnd = scheme.skip(s.string, index) ?: continue
...
private fun atFinalJam(s: IRCString, index: Int): Boolean {
    for (i in index until s.string.length) {
        val c = s.string[i]
        when (c) {
            ' ', '<', '>' -> return true
            ')', '.', ',', ':', ';', '!', '?', '"', '\'' -> continue
...
private fun IRCString.deformat(start: Int, end: Int) =
    string.substring(start, end).filter { it !in formatChars }
